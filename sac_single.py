...
class Args:
    ...
    wandb_project_name: str = "single_task"
...
class SoftQNetwork(nn.Module):
    def __init__(self, env, use_norm=True, hidden=256, activation=F.relu):
        super().__init__()
        obs_dim = int(np.array(env.single_observation_space.shape).prod())
        act_dim = int(np.prod(env.single_action_space.shape))
        self.hidden = hidden
        self.activation=activation
        self.fc1 = nn.Linear(obs_dim + act_dim, self.hidden)
        self.fc2 = nn.Linear(self.hidden, self.hidden)
        self.fc3 = nn.Linear(self.hidden, 1)

        self.norm1 = nn.LayerNorm(self.hidden) if use_norm else nn.Identity()
        self.norm2 = nn.LayerNorm(self.hidden) if use_norm else nn.Identity()
...
class Actor(nn.Module):
    def __init__(self, env, use_norm=True, hidden=256, activation=F.relu):
        ...
        self.hidden = hidden
        self.activation=activation
        self.fc1 = nn.Linear(obs_dim, self.hidden)
        self.fc2 = nn.Linear(self.hidden, self.hidden)
...
"""
    ...
    args = tyro.cli(Args)
    run_name = f"{args.env_id}_{args.exp_name}_{args.seed}_train_{task_nums2str(args.train_tasks)}_test_{task_nums2str(args.test_tasks)}"
    print("run name:", run_name)
    log_path = os.path.join(args.log_dir, args.exp_name,
                            f"task-{task_nums2str(args.test_tasks)}",
                            f"seed-{args.seed}")
    ...
    for step in range(args.total_timesteps + 1):
        ...
        if step > args.learning_starts:
            ...
            with torch.no_grad():
                ...
                qf1_next_target = qf1_target(data.next_observations, next_state_actions).view(-1)
            ...
            qf_loss = (qf1_loss + qf2_loss) / 1000
            ...
            if step % args.test_freq == 0:
                ...
                while not np.all(test_done):
                    ...
                    if line_keys is None:
                        first_info = test_info[0]
                        line_keys = list(first_info['res_p_line'].keys())
                        num_lines = len(line_keys)
                        sums = np.zeros((num_tests, num_lines), dtype=float)
                        counts = np.zeros((num_tests, num_lines), dtype=int)
                    ...
                    if test_done.any():
                        ...
                        for i, task_id in enumerate(args.test_tasks):
                            ...
                            avg_per_line = {}
                            for j, lk in enumerate(line_keys):
                                avg_power = float(sums[i, j] / counts[i, j]) if counts[i, j] > 0 else 0.0
                                avg_per_line[lk] = avg_power
