...
from safepo.common.env import make_ieee123_saferl_env
from safepo.common.logger import EpochLogger
from safepo.common.model import ActorVCritic
from safepo.utils.config import single_agent_args
from safepo.utils.util import task_nums2str
...
def main(args, cfg_env=None):
    ...
    env, obs_space, act_space = make_ieee123_saferl_env(num_envs=args.num_envs, use_vec=True)
    eval_env, _, _ = make_ieee123_saferl_env(num_envs=len(args.test_tasks), use_vec=True)
    config = default_cfg
    # set training steps
    ...
    obs, _ = env.reset(task_nums=np.random.choice(args.train_tasks, size=(args.num_envs,)))
    ...
    for epoch in range(epochs):
        ...
        for steps in range(local_steps_per_epoch):
            ...
            if all(terminated) or all(truncated):
                info["final_observation"] = np.zeros(obs.shape)
        ...
        if args.use_eval:
            for _ in range(eval_episodes):
                eval_done = False
                eval_obs, _ = eval_env.reset(task_nums=args.test_tasks, is_train=False)
                eval_obs = torch.as_tensor(eval_obs, dtype=torch.float32, device=device)
                eval_rew, eval_cost, eval_len = 0.0, 0.0, 0.0
                while not eval_done:
                    with torch.no_grad():
                        eval_act, _, _, _ = policy.step(eval_obs, deterministic=True)
                    eval_next_obs, eval_reward, eval_cost, eval_terminated, eval_truncated, eval_info = eval_env.step(
                        eval_act.detach().cpu().numpy()
                    )
                    eval_next_obs = torch.as_tensor(eval_next_obs, dtype=torch.float32, device=device)
                    eval_rew += eval_reward
                    eval_cost += eval_cost
                    eval_len += 1
                    eval_done = all(eval_terminated) or all(eval_truncated)
                    eval_obs = eval_next_obs
                
                eval_dict = {}
                for task_i in range(len(args.test_tasks)):
                    eval_dict[f"Metrics/EvalEpRet_task{args.test_tasks[task_i]}"] = eval_rew[task_i]
                    eval_dict[f"Metrics/EvalEpCost_task{args.test_tasks[task_i]}"] = eval_cost[task_i]
                    eval_dict[f"Metrics/EvalEpLen_task{args.test_tasks[task_i]}"] = eval_len
        ...
        if not logger.logged:
            ...
            if args.use_eval:
                for eval_task in args.test_tasks:
                    logger.log_tabular(f"Metrics/EvalEpRet_task{eval_task}")
                    logger.log_tabular(f"Metrics/EvalEpCost_task{eval_task}")
                    logger.log_tabular(f"Metrics/EvalEpLen_task{eval_task}")
...
if __name__ == "__main__":
    ...
    task_dir = f"train-{task_nums2str(args.train_tasks)}-test-{task_nums2str(args.test_tasks)}"
    args.log_dir = os.path.join(args.log_dir, args.experiment, args.task, algo, task_dir, relpath)
