...
class Args:
    ...
    wandb_project_name: str = "grid_moe"
    ...
    log_dir: str = "grid_moe"
    train_tasks: tuple[int, ...] = (15,)
    test_tasks: tuple[int, ...] = (15,)
    add_edge: bool = False
    mix_num: int = 10
    """the number of overall experts"""
    topk: int = 4
...
class SoftQNetwork(nn.Module):
    def __init__(self, env, mix_num, topk, use_norm=True, hidden=128, activation=F.relu):
        super().__init__()
    ...
    def forward(self, x, a):
        ...
        routed_gate_scores = gate_scores[:, 1:]
        topk_vals, topk_idx = torch.topk(routed_gate_scores, k=self.topk - 1, dim=-1)
        topk_idx = topk_idx + 1

        with torch.no_grad():
            self.total_calls += batch
            self.expert_usage[0] += batch
        ...
        scores = torch.zeros(batch, self.mix_num, device=x.device)
        scores[:, 0] = 1.0
        ...
        x = self.fc3(x)
        importance_i = torch.sum(routed_gate_scores, dim=0) / batch
        mean_importance = torch.mean(importance_i)
        importance_variance = 10 * torch.mean((importance_i - mean_importance) ** 2)
        selected_gate_scores = torch.zeros_like(routed_gate_scores)
        for i in range(batch):
            selected_indices = topk_idx[i] - 1
            selected_gate_scores[i, selected_indices] = routed_gate_scores[i, selected_indices]
        load_i = torch.sum(selected_gate_scores, dim=0)
        mean_load = torch.mean(load_i)
        load_std = torch.sqrt(torch.mean((load_i - mean_load) ** 2))
        load_cv2 = (load_std / mean_load) ** 2
        return x, importance_variance, load_cv2
...
class Actor(nn.Module):
    ...
    def forward(self, x):
        ...
        load_std = torch.sqrt(torch.mean((load_i - mean_load) ** 2))
        load_cv2 = (load_std / mean_load) ** 2
        return mean, log_std, importance_variance, load_cv2

    def get_action(self, x):
        mean, log_std, importance_variance, load_cv2 = self(x)
        std = log_std.exp()
        ...
        return action, log_prob, importance_variance, load_cv2
    ...
    for step in range(args.total_timesteps + 1):
        ...
        if step > args.learning_starts:
            ...
            with torch.no_grad():
                ...
                qf1_next_target, _, _ = qf1_target(data.next_observations, next_state_actions)
                qf2_next_target, _, _ = qf2_target(data.next_observations, next_state_actions)
                min_qf_next_target = torch.min(qf1_next_target, qf2_next_target) - alpha * next_state_log_pi
                next_q_value = data.rewards.flatten() + (1 - data.dones.flatten()) * args.gamma * (
                    min_qf_next_target).view(-1)

            qf1_a_values, qf1_imp_loss, qf1_load_loss = qf1(data.observations, data.actions)
            qf2_a_values, qf2_imp_loss, qf2_load_loss = qf2(data.observations, data.actions)
            qf1_a_values = qf1_a_values.view(-1)
            qf2_a_values = qf2_a_values.view(-1)
            qf1_loss = F.mse_loss(qf1_a_values, next_q_value)
            qf2_loss = F.mse_loss(qf2_a_values, next_q_value)
            main_loss = (qf1_loss + qf2_loss) / 1000
            aux_loss_raw = qf1_imp_loss + qf2_imp_loss + qf1_load_loss + qf2_load_loss
            aux_loss = aux_loss_raw * args.critic_aux_loss_weight
            qf_loss = main_loss + aux_loss
            ...
            if step % args.policy_frequency == 0:  # TD 3 Delayed update support
                for _ in range(
                    ...
                    pi, log_pi, actor_imp_loss, actor_load_loss = actor.get_action(data.observations)
                    qf1_pi, _, _ = qf1(data.observations, pi)
                    qf2_pi, _, _ = qf2(data.observations, pi)
                    min_qf_pi = torch.min(qf1_pi, qf2_pi)
                    main_loss = ((alpha * log_pi) - min_qf_pi).mean()
                    aux_loss_raw = actor_imp_loss + actor_load_loss
                    aux_loss = aux_loss_raw * args.actor_aux_loss_weight
                    actor_loss = main_loss + aux_loss
            ...
            if step % args.test_freq == 0:
                ...
                while not np.all(test_done):
                    with torch.no_grad():
                        ...
                        test_action, _, _, _ = test_actor.get_action(torch.Tensor(test_obs).to(device))
                ...
                writer.add_scalar('expert_usage/shared_expert', expert_usage[0].item(), global_step)
                writer.add_scalar('expert_usage/routed_experts_avg', torch.mean(expert_usage[1:]).item(), global_step)

                logger.log_tabular('expert_usage/routed_experts_std', torch.std(expert_usage[1:]).item())
                logger.log_tabular('expert_usage/shared_expert', expert_usage[0].item())
                logger.log_tabular('expert_usage/routed_experts_avg', torch.mean(expert_usage[1:]).item())
